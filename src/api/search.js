...
const queryAlias = async (db, val) => db.map('SELECT alias FROM aliases WHERE id IN (SELECT max(id) FROM aliases GROUP BY alias) AND hash IS NOT NULL AND alias LIKE LOWER($1);', [ `%${val}%` ], (r) => r.hash);
